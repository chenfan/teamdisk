...
function ss_start()
{
    ...
    if(!ss($sn))
    {
        ...
        if(!ss('remote_ip')) ss_set('remote_ip',$_SERVER['REMOTE_ADDR']);
        ss_set($sn,session_id());
        if(ss('weibo_user_info')) setcookie('weibo_bind','1');
    }
    else
    {
    }
...
function ajax_box( $content , $title = '系统消息' , $close_time = 0 , $forward = '' ,$vc=0,$width=400)
{

    require_once( CROOT . 'view/layout/ajax/box.tpl.html' );
}

function ajax_box2( $content , $title = '系统消息' ,$tpl="err", $close_time = 0 , $forward = '' )
{
    require_once( CROOT . 'view/layout/ajax/'.$tpl.'.tpl.html' );
}

function ajax_json( $info ,$status=NULL,$title=NULL,$data=NULL)
{	
    if(!is_array($info))
        $info=array('msg'=>$info);
    if(!isset($info['title'])) $info['title'] = is_null($title) ? '系统消息' : $title;
    if(!isset($info['status'])) $info['status']= is_numeric($status) ? $status : 0;
...
function ajax_render($dat,$tpl='',$dir=false,$ret=false)
{
    ...
    elseif($tpl) $tpl=AROOT.'view/layout/ajax/'.$dir.'/'.$tpl.'.tpl.html';
    else $tpl=AROOT.'view/layout/ajax/'.$dir.'/'.t(g('a')).'.tpl.html';
    require_once($tpl);
}
